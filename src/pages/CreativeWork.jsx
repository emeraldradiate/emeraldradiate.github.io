...
function CreativeWork() {
  ...
  // filter projects to only include tagged "Creative Work"
  const creativeProjects = projects.filter(project => 
    project.tags.includes("Creative Work")
  );

  // get all unique tags from projects (excluding "Creative Work" itself)
  const availableTags = useMemo(() => {
    const tagSet = new Set();
    creativeProjects.forEach(project => {
      project.tags.forEach(tag => {
        if (tag !== "Creative Work") {
          tagSet.add(tag);
        }
      });
    });
    ...
    <>
      ...
      <div className="min-h-screen bg-black">
        ...
        <section className="relative px-6 md:px-12 lg:px-24 xl:px-40 pt-32 pb-4">
          ...
          {/* rotating sparkle */}
          <div className="absolute sparkle-spin left-12 lg:left-24 xl:left-55" style={{top: '140px', zIndex: 0, opacity: 0.3}}>
          ...
          <div className="relative max-w-5xl mx-auto text-right">
            ...
            <p className="text-sm mb-8" style={{fontFamily: "'Lora', serif", color: '#fff', maxWidth: '600px', marginLeft: 'auto'}}>
              A collection of my creative projects, including user interface design, brand identity creation, and digital illustration/animation.
            </p>
          </div>
        </section>
